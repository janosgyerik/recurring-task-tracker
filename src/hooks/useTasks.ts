...
function calculateNextDue(lastCompleted: Date, frequency: Frequency, weekDays?: number[], months?: number[], interval: number = 1): Date {
  ...
  if (months && months.length > 0) {
    ...
    if (nextMonthIndex !== -1) {
      ...
      // Wrap to next year
      targetDate = addYears(setMonth(setDate(baseDate, 1), sortedMonths[0]), 1);
    }
    ...
    if (nextDayIndex !== -1) {
      ...
      switch (frequency) {
        case 'day':
          ...
          break;
        case 'week':
          nextIntervalStart = addWeeks(baseDate, interval);
          break;
        case 'month':
